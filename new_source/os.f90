...
module os_mod
  ...
  use string_mod, only : sl,split,replace
...
function argument_parser_constructor() result(obj)
  ...
  do i=2,command_argument_count()
    ...
    if (keyvalue(1) .eq. 'output') then
      if(has_provided_output) then
        write(error_unit,'("[",A,":",I0,"]: More than one output provided on argument list.")') __FILE__,__LINE__
        error stop  
      endif
  ...
  do i=1,size(obj%namelists(:,1))
    ...
    if(.not. is_string_in_file(file,'&'//trim(nml))) then
      write(error_unit,'("[",A,":",I0,"]: Namelist ",A," not present in file ",A)') __FILE__,__LINE__,trim(nml),trim(file)
      error stop
    endif
