...
contains
  ...
  subroutine build_from_file(this)
    ...
    logical :: all_inequivalent, ws_all, mix_all, &
  ...
  subroutine restore_to_default(this,full)
    ...
    if (present(full) .and. full .and. associated(this%lattice)) call this%lattice%restore_to_default(full)
         ...
         do INUM = 2,4,2
            ...
            do IR = 2,NR
               ...
               do IR1 = 2,IR
                  ...
                  SUM1 = SUM1 + &
                     WGT*DRDI*this%FUN2(IR1,3,ISI)*(ROFI(IR1)**INUM)/(ROFI(IR)**INUM1)
               end do
               ...
               do IR1 = IR,NR
                  ...
                  DRDI = atom%A * (ROFI(IR1)+B)
                  SUM2 = SUM2 + &
                     WGT*DRDI*this%FUN2(IR1,3,ISI)*(ROFI(IR)**INUM)/(ROFI(IR1)**INUM1)
               end do
               !-----------------------------------------
               S12 = SUM1 + SUM2
               WGT = 2 * (MOD(IR+1,2)+1) / 3.0d0
