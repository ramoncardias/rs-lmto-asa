...
module charge_mod
  ...
  use lattice_mod
  use self_mod
  use math_mod, only: pi, sqrt_pi, cross_product, ang2au
...
contains
  ...
  subroutine bulkmat(this)
    ...
    if (isopen) then
      write(error_unit,'(a)') 'charge%bulkmat, file ves.out: Unit 10 is already open'
      error stop
    else
    ...
    if (isopen) then
      write(error_unit,'(a)') 'charge%bulkmat, file mad.mat: Unit 11 is already open'
      error stop
    else
 ...
 &   this%W(J7RLAT),NKR,this%W(J7DLAT),NKD,this%W(J7AMAD))
  ...
  subroutine surfmat(this)
    ...
    bk(1,:) = cross_product(bs(2,:),bs(3,:))
    bk(2,:) = cross_product(bs(3,:),bs(1,:))
    bk(3,:) = cross_product(bs(1,:),bs(2,:))

    this%vol = abs(this%bsx(1)*bk(1,1)+this%bsy(1)*bk(2,1)+this%bsz(1)*bk(3,1))

    write(*,*) this%bsx(1)*bk(1,1), this%bsy(1)*bk(1,2), this%bsz(1)*bk(1,3)
    write(*,*) bs(:,1)
    write(*,*) bs(:,2)
    write(*,*) bs(:,3)
  ...
  subroutine build_alelay(this)
    ...
    select case(this%lattice%crystal_sym)
      case('bcc')
      select case(this%lattice%surftype)
        case('001')
          this%nq3 = 1

          allocate(this%qx3(this%nq3),this%qy3(this%nq3),this%qz3(this%nq3))

          this%bsx(:) = [0.5d0, 0.5d0, 0.5d0]
          this%bsy(:) = [-0.5d0, 0.5d0, 0.0d0]
          this%bsz(:) = [0.0d0, 0.0d0, 0.5d0]
